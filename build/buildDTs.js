...
const overrideCollectionData = await fs.promises.readFile(
...
const exportData = data
  ...
  .replace(
    /<{(.*)} T>/g,
    (all, toExtend) => `<T extends ${toExtend.replace('*', 'any')}>`,
  )
  ...
  .replace(
    /extends import\("ol"\).Feature<import\("ol\/geom\/Geometry"\).default>/,
    'extends olFeature<Geometry>',
  )
  .replace(/^export\sconst\senum/gm, 'export enum')
  .replace(/^(export\sconst.*:\s?)symbol/gm, '$1unique symbol');

const overrideCollection = overrideCollectionData
  .toString()
  .replace(/import[^;]*;/, '');

const editor = editorData.toString().replace(/import.*;/, '');
...
function verifyTsd() {
  return new Promise((resolve, reject) => {
    const child = spawn(/^win/i.test(process.platform) ? 'npx.cmd' : 'npx', [
      'tsc',
      '-p',
