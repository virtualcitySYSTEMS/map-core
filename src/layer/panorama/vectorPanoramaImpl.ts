...
function setupMapSourceListeners(
  ...
  const sourceListeners = [
    originalSource.on('addfeature', (event) => {
      if (featureWithinImage(event.feature, map.currentPanoramaImage)) {
  ...
  const panoramaImageListener = map.currentImageChanged.addEventListener(() => {
  ...
  implements FeatureLayerImplementation
{
  ...
  constructor(map: PanoramaMap, options: VectorImplementationOptions) {
    ...
    this._sourceListeners = setupMapSourceListeners(
      map,
      options.source,
      this.source,
    );
