...
import getTiledObliqueImageServer, {
...
describe('ObliqueCollection', () => {
  ...
  describe('adding datasets', () => {
    ...
    describe('in constructor', () => {
      ...
      it('should add passed in serialized data sets', () => {
        ...
        expect(obliqueCollection.dataSets[0]).to.be.an.instanceOf(
          ObliqueDataSet,
        );
    ...
    describe('with addDataSet', () => {
      ...
      describe('with a loaded collection', () => {
        ...
        it('should add tile features, if the tiled feature source exists', async () => {
          ...
          await obliqueCollection.addDataSet(dataSet);
          expect(tileFeatureSource.getFeatures()).to.have.lengthOf(
            Object.keys(dataSet.getTiles()).length,
          );
        });
      });
    });
  ...
  describe('loading the collection', () => {
    ...
    it('should add tiled features to the tiled vector source', async () => {
      ...
      await obliqueCollection.load();
      expect(tileFeatureSource.getFeatures()).to.have.lengthOf(
        Object.keys(obliqueCollection.getTiles()).length,
      );
    });
  ...
  describe('querying available tiles', () => {
    ...
    it('should return all available tiles', () => {
      ...
      expect(Object.keys(tiles)).to.have.members(
        Object.keys(dataSet.getTiles()),
      );
    });
  ...
  describe('querying data state', () => {
    ...
    it('should return the state of a coordinate', async () => {
      expect(
        obliqueCollection.getDataStateForCoordinate(tiledMercatorCoordinate),
    ...
    it('should return the state of an extent', async () => {
      expect(obliqueCollection.getDataStateForExtent(mercatorExtent)).to.equal(
        DataState.PENDING,
      );
  ...
  describe('querying available Viewdirections', () => {
    ...
    it('should return all available viewdirections', async () => {
      await obliqueCollection.loadDataForCoordinate(tiledMercatorCoordinate);
      expect(obliqueCollection.getAvailableViewDirections()).to.have.members([
        1, 2, 3, 4,
      ]);
    });
  });
  ...
  describe('querying images by coordinate', () => {
    ...
    describe('sync', () => {
      ...
      it('should return the image to the coordinate', () => {
        const image = obliqueCollection.getImageForCoordinate(
          tiledMercatorCoordinate2,
    ...
    describe('async', () => {
      it('should load the data for a given coordinate first', async () => {
        const image = await obliqueCollection.loadImageForCoordinate(
          tiledMercatorCoordinate2,
          ObliqueViewDirection.NORTH,
        );
        expect(image.name).to.equal('025_070_119003689');
      ...
      it('should determine, if the closest image intersects the coordinate given', async () => {
        const hasImage = await obliqueCollection.hasImageAtCoordinate(
          tiledMercatorCoordinate2,
          ObliqueViewDirection.NORTH,
        );
        ...
        const hasNoImage = await obliqueCollection.hasImageAtCoordinate(
          coord,
          ObliqueViewDirection.NORTH,
        );
      ...
      it('should load images adjacent to a given image', async () => {
        ...
        const adjacentImage = await obliqueCollection.loadAdjacentImage(
          image,
          0,
        );
      ...
      it('should return undefined, if there is no image adjacent to a given image', async () => {
        const coord = [
          tiledMercatorCoordinate2[0] - 50000,
          tiledMercatorCoordinate2[1] + 50000,
        ];
        const image = await obliqueCollection.loadImageForCoordinate(
          coord,
          ObliqueViewDirection.NORTH,
  ...
  describe('provided feature sources', () => {
    ...
    describe('for images', () => {
      ...
      it('should have a feature for each image', () => {
        ...
        expect(imageFeatureSource.getFeatures()).to.have.lengthOf(
          obliqueCollection.images.length,
        );
      });
  ...
  describe('serialization', () => {
    ...
    describe('of a configured collection', () => {
      ...
      it('should configure scaleFactor', () => {
        expect(outputConfig).to.have.property(
          'scaleFactor',
          inputConfig.scaleFactor,
        );
      });

      it('should serialize dataSets', () => {
        expect(outputConfig.dataSets).to.be.an('array').and.have.lengthOf(3);
        expect(outputConfig.dataSets[0]).to.have.property(
          'url',
          inputConfig.dataSets[0].url,
        );
        expect(outputConfig.dataSets[1]).to.have.property(
          'url',
          inputConfig.dataSets[1].url,
        );
        expect(outputConfig.dataSets[1])
          .to.have.property('projection')
          .and.to.have.property(
            'epsg',
            inputConfig.dataSets[1].projection.epsg,
          );
        expect(outputConfig.dataSets[2]).to.have.property(
          'url',
          inputConfig.dataSets[2].url,
        );
        expect(outputConfig.dataSets[2])
          .to.have.property('terrainProvider')
          .and.to.have.property(
            'url',
            inputConfig.dataSets[2].terrainProvider.url,
          );
      });
    });
  });
