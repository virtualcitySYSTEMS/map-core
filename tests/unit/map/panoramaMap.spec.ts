...
import { Collection, Projection, Viewpoint } from '../../../index.js';
import { getVcsEventSpy } from '../helpers/cesiumHelpers.js';

function createStubbedDataset(
  name: string,
  features: Feature[],
): {
  ...
  const dataset = new PanoramaDataset({ name, url: '' });
...
describe('PanoramaMap', () => {
  ...
  describe('getting the closest image to a position', () => {
    ...
    before(() => {
      map = getPanoramaMap();
      const dataset1 = createStubbedDataset('dataset1', [
        new Feature({
          name: 'test',
          time: 1,
      ...
      const dataset2 = createStubbedDataset('dataset2', [
        new Feature({
          name: 'test2',
          time: 1,
      ...
      map.panoramaDatasets.add(dataset1.dataset);
      map.panoramaDatasets.add(dataset2.dataset);
    ...
    beforeEach(() => {
      [...map.panoramaDatasets].forEach((dataset) => {
        dataset.activate();
      });
    });
    ...
    it('should return undefined, if the dataset isnt active', async () => {
      const dataset = map.panoramaDatasets.getByKey('dataset1')!;
      dataset.deactivate();
  ...
  describe('setting a new panorama dataset collection', () => {
    let map: PanoramaMap;
    ...
    it('should raise the collection changed event', () => {
      const spy = getVcsEventSpy(map.panoramaDatasetsChanged);
      map.panoramaDatasets = new Collection<PanoramaDataset>();
      expect(spy).to.have.been.calledOnce;
    });

    it('should not destroy a collection not owned by the map', () => {
      const collection = Collection.from([new PanoramaDataset({ url: '' })]);
      map.panoramaDatasets = collection;
      map.destroy();
  ...
  describe('setting a new panorama image', () => {
    ...
    it('should raise the image changed event', () => {
      const spy = getVcsEventSpy(map.currentImageChanged);
      map.setCurrentImage(image);
