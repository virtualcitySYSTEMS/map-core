...
import PanoramaTileHighlight from '../../../src/interaction/panoramaTileHighlight.js';
...
describe('PanoramaTileHighlight', () => {
  let map: PanoramaMap;
  let interaction: PanoramaTileHighlight;
  ...
  beforeEach(() => {
    interaction = new PanoramaTileHighlight();
  ...
  describe('without a highlighted panorama feature', () => {
    it('should highlight the panorama feature', async () => {
      ...
      expect(
        dataset.layer.featureVisibility.highlightedObjects,
      ).to.have.property('testPanorama');
    });
    ...
    it('should not highlight random features', async () => {
      ...
      await interaction.pipe({ ...testEvent });
      expect(dataset.layer.featureVisibility.highlightedObjects).to.be.empty;
    });
  });
  ...
  describe('with a highlighted panorama feature', () => {
    ...
    it('should not highlight a panorama feature with the same name', async () => {
      const spy = sinon.spy(dataset.layer.featureVisibility, 'highlight');
      pickStub.returns({
        id: { olFeature: feature },
      });
      await interaction.pipe({ ...testEvent });
