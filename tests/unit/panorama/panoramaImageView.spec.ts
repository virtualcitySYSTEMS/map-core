...
describe('PanoramaImageView', () => {
  describe('tile loading and rendering', () => {
    ...
    it('should render tiles based on camera and image', () => {
      ...
      const tileCoordinates = createVisibleTiles.getCall(0)
        .args[0] as PanoramaTileCoordinate[];
  ...
  describe('image change reaction', () => {
    ...
    describe('unsetting the image', () => {
      ...
      it('should set the empty image overlay', () => {
        const div = document.createElement('div');
        div.appendChild(map.getCesiumWidget().scene.canvas);
        const currentChildren =
          map.getCesiumWidget().scene.canvas.parentElement!.children.length;
        map.setCurrentImage();
        expect(
          map.getCesiumWidget().scene.canvas.parentElement!.children.length,
        ).to.equal(currentChildren + 1);
      });

      it('should remove the empty image overlay on new image', () => {
        const div = document.createElement('div');
        div.appendChild(map.getCesiumWidget().scene.canvas);
        const currentChildren =
          map.getCesiumWidget().scene.canvas.parentElement!.children.length;
