...
import type { PanoramaDatasetOptions } from '../../../src/panorama/panoramaDataset.js';
...
describe('PanoramaDataset', () => {
  describe('state handling', () => {
    ...
    describe('activating', () => {
      it('it should activate, if not active', () => {
        dataset.activate();
        expect(dataset.active).to.be.true;
      });

      it('should emit the activated event', () => {
        const spy = getVcsEventSpy(dataset.stateChanged);
        dataset.activate();
        expect(spy).to.have.been.calledOnce;
      });

      it('should only emit state changed, if the state actually changes', () => {
        const spy = getVcsEventSpy(dataset.stateChanged);
        dataset.activate();
        dataset.activate();
        expect(spy).to.have.been.calledOnce;
    ...
    describe('deactivating', () => {
      ...
      it('should emit the deactivated event', () => {
        const spy = getVcsEventSpy(dataset.stateChanged);
        dataset.deactivate();
        expect(spy).to.have.been.calledOnce;
      });
    ...
    describe('loosely coupled layer state', () => {
      it('should activate the dataset, if the layer gets activated', async () => {
        await dataset.layer.activate();
        expect(dataset.active).to.be.true;
      });

      it('should deactivate the layer, when deactivating the dataset', async () => {
        await dataset.layer.activate();
        dataset.deactivate();
        expect(dataset.active).to.be.false;
      });
  ...
  describe('config', () => {
    ...
    describe('of a configured dataset', () => {
      ...
      it('should configure the active on startup', () => {
        expect(output).to.have.property(
          'activeOnStartup',
          input.activeOnStartup,
        );
      });
