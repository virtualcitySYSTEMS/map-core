...
import { Vector, Openlayers, AbstractFeatureProvider } from '../../../../index.js';
import FeatureProviderInteraction from '../../../../src/vcs/vcm/interaction/featureProviderInteraction.js';

async function addLayerWithFeatureProvider(layerCollection, getFeatureByCoordinate) {
  const layer = new Vector({});
  layer.featureProvider = new AbstractFeatureProvider(layer.name, { mapTypes: [Openlayers.className] });
...
describe('FeatureProviderInteraction', () => {
  ...
  it('should return the first feature if returning an array', async () => {
    const feature = new Feature();
    await addLayerWithFeatureProvider(map.layerCollection, () => [feature, new Feature()]);
    const event = getDummyEvent();
    await interaction.pipe(event);
    expect(event).to.have.property('feature', feature);
  });
  ...
  it('should not overwrite a feature, if the event already has a feature', async () => {
    const feature = new Feature();
    await addLayerWithFeatureProvider(map.layerCollection, () => [new Feature()]);
    const event = getDummyEvent();
  ...
  it('should respect the layers zIndex', async () => {
    const feature = new Feature();
    const layer = await addLayerWithFeatureProvider(map.layerCollection, () => [feature]);
    layer.zIndex = 2;
    await addLayerWithFeatureProvider(map.layerCollection, () => [new Feature()]);
    const event = getDummyEvent();
    await interaction.pipe(event);
