...
} from '../../../../src/vcs/vcm/util/geometryHelpers.js';

describe('vcs.vcm.util.geometryHelpers', () => {
  ...
  describe('getFlatCoordinatesFromSimpleGeometry', () => {
    ...
    it('should return the coordinates of a MultiPoint', () => {
      geometry = new MultiPoint([[1, 2], [2, 2]]);
      flatCoordinates = getFlatCoordinatesFromSimpleGeometry(geometry);
      expect(flatCoordinates).to.have.deep.members([[1, 2], [2, 2]]);
    });

    it('should return the coordinates of a 3D polygon', () => {
      geometry = new Polygon([[[50, 50, 3], [50, 55, 3], [55, 50, 3]]]);
      flatCoordinates = getFlatCoordinatesFromSimpleGeometry(geometry);
      expect(flatCoordinates).to.have.deep.members([[50, 50, 3], [50, 55, 3], [55, 50, 3]]);
    });

    it('should return the coordinates of a 2D polygon', () => {
      geometry = new Polygon([[[50, 50], [50, 55], [55, 50]]]);
      flatCoordinates = getFlatCoordinatesFromSimpleGeometry(geometry);
      expect(flatCoordinates).to.have.deep.members([[50, 50], [50, 55], [55, 50]]);
    });

    it('should return the coordinates of a MultiPolygon', () => {
      geometry = new MultiPolygon([[[[50, 50], [50, 55], [55, 50]]], [[[50, 50], [50, 55], [55, 50]]]]);
      flatCoordinates = getFlatCoordinatesFromSimpleGeometry(geometry);
      expect(flatCoordinates).to.have.deep.members([[50, 50], [50, 55], [55, 50], [50, 50], [50, 55], [55, 50]]);
    });

    it('should return the coordinates of a 3D linestring', () => {
      geometry = new LineString([[1, 2, 3], [1, 2, 3]]);
      flatCoordinates = getFlatCoordinatesFromSimpleGeometry(geometry);
      expect(flatCoordinates).to.have.deep.members([[1, 2, 3], [1, 2, 3]]);
    });

    it('should return the coordinates of a 2D linestring', () => {
      geometry = new LineString([[1, 2], [1, 3]]);
      flatCoordinates = getFlatCoordinatesFromSimpleGeometry(geometry);
      expect(flatCoordinates).to.have.deep.members([[1, 2], [1, 3]]);
    });

    it('should return the coordinates of a MultilineString', () => {
      geometry = new MultiLineString([[[1, 2], [1, 3]], [[1, 4], [1, 5]]]);
      flatCoordinates = getFlatCoordinatesFromSimpleGeometry(geometry);
      expect(flatCoordinates).to.have.deep.members([[1, 2], [1, 3], [1, 4], [1, 5]]);
    });
  });
  ...
  describe('getFlatCoordinatesFromGeometry', () => {
    ...
    before(() => {
      geometries = {
        ...
        LineString: new LineString([[1, 1, 1], [2, 2, 2]]),
        MultiPoint: new MultiPoint([[1, 1, 1], [2, 2, 2]]),
        Polygon: new Polygon([
          [[0, 0, 0], [10, 0, 0], [10, 10, 0], [0, 10, 0]],
          [[2, 2, 0], [0, 8, 0], [8, 8, 0], [8, 0, 0]],
        ]),
        MultiLineString: new MultiLineString([[[1, 1, 1], [2, 2, 2]], [[1, 1, 1], [3, 3, 3]]]),
        Circle: new Circle([0, 0, 0], 1, 'XYZ'),
        MultiPolygon: new MultiPolygon([
          [[[0, 0, 0], [1, 0, 0], [0, 1, 0]]],
          [[[2, 2, 0], [2, 5, 0], [-2, 5, 0]], [[1, 3, 0], [0, 4, 0], [1, 4, 0]]],
        ]),
      };
      ...
      flatCoordinates = {
        ...
        Polygon: [[0, 0, 0], [10, 0, 0], [10, 10, 0], [0, 10, 0], [2, 2, 0], [0, 8, 0], [8, 8, 0], [8, 0, 0]],
        MultiLineString: [[1, 1, 1], [2, 2, 2], [1, 1, 1], [3, 3, 3]],
        Circle: [[0, 0, 0], [1, 0, 0]],
        MultiPolygon: [
          [0, 0, 0], [1, 0, 0], [0, 1, 0],
          [2, 2, 0], [2, 5, 0], [-2, 5, 0], [1, 3, 0], [0, 4, 0], [1, 4, 0],
    ...
    it('should get the flat coordinates with a reference to the original', () => {
      Object.keys(geometries).forEach((key) => {
        const coords = key === 'Circle' ?
          [geometries[key].getFirstCoordinate(), geometries[key].getLastCoordinate()] :
